...
int main(int argc, char *argv[ ])
{
    ...
    if(argc < 6){
        ...
        fprintf(stdout, "Shrooms NetBIOS Scanner\nUsage: %s <ip range start (1.0.0.0)> <ip range end (255.255.255)> <outfile> <threads> <scan delay in ms>\n", argv[0]);
        exit(-1);
    }
    fd = fopen(argv[3], "a");
