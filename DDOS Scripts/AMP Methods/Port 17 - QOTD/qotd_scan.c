...
gcc -lpthread v2.c -lpcap -o ssdps
...
{
	if(argc < 6){
		fprintf(stderr, "Invalid parameters for Template Scan (edited cancer by Phenomite)!\n");
		fprintf(stdout, "Usage: %s <range start> <range end> <out> <threads> <delay in ms>\n", argv[0]);
		exit(-1);
	}
	fd = fopen(argv[3], "a");
	...
	printf("Opening inet %s for naughties ... " , devname);
